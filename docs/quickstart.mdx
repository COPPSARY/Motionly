...
- Import an audio file from the **Audio** rail item, then drag it onto the timeline to place it. The bottom track shows a waveform preview and you can drag horizontally to adjust its start time.

Audio placement is persisted in `.motion` and plays synchronized with preview. Audio is not yet mixed into MP4 export.
