---
title: Text layers in .motion files
description: Create editable text layers in Motionly .motion files, set font, size, weight, color, and alignment, and prepare copy for text reveal animations.
---

# Text

Text objects use the `text` keyword and a unique name.

```motion
text title {
  value "Motion graphics"
  center
  x 0
  y 0
  size 72
  color #ffffff
  opacity 1
}
```

Use `size`, not `fontSize`.

Keep text readable. Split long copy into aligned text layers only when it improves layout, and preserve exact words and punctuation when matching narration.

