...
```bash
npx motionly init my-video --provider opencode   # one agent, no prompt
npx motionly init my-video --all                 # every supported agent
npx motionly init my-video --skip-skills          # no agent skills
```
...
Each install includes the same `references/` library. Re-running keeps existing files and never overwrites your edits.
