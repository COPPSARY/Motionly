...
describe('regular element clip timing', () => {
  it('reports storyboard element windows in the same global time used by preview evaluation', () => {
    const scene = buildSceneGraph(
      parseMotion(`
        canvas { duration 6s }
        scene demo { start 3.4s duration 1s }
        text title {
          scene demo
          value "Generated by AI"
          duration 300ms
          textAnimation "charReveal(duration 200ms)"
        }
      `)
    );

    expect(elementTimelineRange(scene, 'title', scene.canvas.duration)).toEqual({
      start: 3.4,
      end: 3.7,
    });
    expect(
      evaluateScene(scene, 3.69).elements.some(
        (element) => element.properties.textGroup === 'title'
      )
    ).toBe(true);
    expect(
      evaluateScene(scene, 3.7).elements.some((element) => element.properties.textGroup === 'title')
    ).toBe(false);
  });
